...
from analysis.lib.purification import purify_pq as ppq
import numpy as np
import os,h5py
...
import Analysis_params_BK as BK_params;reload(BK_params)
...
class BK_analysis(object):
    ...
    def check_tail_w1_w2(self,st_start = None,p_sep = None,st_len = None, ch1_offset =None):
        """
        ...
        for a in self.lt4_dict['raw_data']:
            ...
            w1_ch1 = self.get_total_number_of_clicks_in_window(a,1,st_start+ch1_offset,st_len)
            w2_ch0 = self.get_total_number_of_clicks_in_window(a,0,st_start+p_sep,st_len)
            w2_ch1 = self.get_total_number_of_clicks_in_window(a,1,st_start+p_sep+ch1_offset,st_len)
    ...
    def correlate_RO_results(self,apply_ROC = False, verbose = True,return_value = False):
        ...
        if apply_ROC:
            ...
            corrected_psi_minus = np.transpose(sscorr.ssro_correct_twoqubit_state_photon_numbers(np.array(m_correlations[::-1]),F0_LT4,F0_LT3,F1_LT4,F1_LT3,verbose=False))
            corrected_psi_plus  = np.transpose(sscorr.ssro_correct_twoqubit_state_photon_numbers(np.array(p_correlations[::-1]),F0_LT4,F0_LT3,F1_LT4,F1_LT3,verbose=False))

            if verbose:
                print '\n Corrected for read out: '
                row_format ="{:>12}"+"{:>12.2f}" * (len(x))

                for state, row in zip(['psi_minus'], corrected_psi_minus):
                    print "-"*(12*4+15)
                    print row_format.format(state+' |', *row)

                print

                for state, row in zip(['psi_plus'], corrected_psi_plus):
                    print "-"*(12*4+15)
                    print row_format.format(state+' |', *row)

            if return_value:
                return np.sum(m_correlations)* corrected_psi_minus[0], np.sum(p_correlations) * corrected_psi_plus[0] # PH a bit bodged for the errors
    ...
    def sweep_filter_parameter_vs_correlations(self,parameter_name,parameter_range):
        ...
        for x in parameter_range:
            ...
            psi_m_corrs, psi_p_corrs = self.correlate_RO_results(verbose=False,apply_ROC = True,return_value = True)
