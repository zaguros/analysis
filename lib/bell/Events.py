...
def get_SSRO_events(pqf, marker_chan ,RO_start, RO_length, chan_rnd_0, chan_rnd_1, sync_time_lim = 0, index = 1, VERBOSE = True):
    ...
    if num_mrkr > 0:  
        ...
        unique_sync_num_with_markers = marker_sync_numbers
        ...
        if VERBOSE:
            print "The number of unique sync numbers that have a marker is:", len(PQ_save_sync_numbers)
            print "The number of events with a sync number that has a marker is:", len(indices_PQ_save_sync_num)
        ...
        for i,s in enumerate(unique_sync_num_with_markers):
            ...
            abs_times_s = PQ_time[is_sync_num_s]
            sync_time_s = PQ_sync_time[is_sync_num_s]
            abs_time_mrkr = abs_times_s[len(abs_times_s)-1]  + (15000 - sync_time_s[len(abs_times_s)-1])
...
def re_analyze_SSRO_data(pqf, marker_chan ,RO_start, RO_length, chan_rnd_0, chan_rnd_1, sync_time_lim = 0,  VERBOSE = True ):
    ...
    unique_sync_num_with_markers = np.unique(PQ_sync_number)
    ...
    for i,s in enumerate(unique_sync_num_with_markers):
        ...
        abs_times_s = PQ_time[is_sync_num_s]
        sync_time_s = PQ_sync_time[is_sync_num_s]
        abs_time_mrkr = abs_times_s[len(abs_times_s)-1]  + (15000 - sync_time_s[-1])
