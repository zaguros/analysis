...
def _aggregated_coincidences(Base_Folder):
    ...
    in_coincidences = np.vstack((in_coincidences, pq_tools.get_coincidences_from_folder(Base_Folder,save=True)))
...
def TPQI_analysis(Base_Folder, ch0_start, ch1_start, WINDOW_LENGTH, dif_win1_win2, noof_pulses, return_sn = False , Verbose = True):
    ...
    if return_sn:
        sn = coincidences[is_sync_time_filter,column_sync_num_ch0]  
        return dts, filtered_dts, sn
    else:
