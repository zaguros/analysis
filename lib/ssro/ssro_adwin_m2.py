...
def run(h5dat, ms, path):
    """
    runs a single SSRO analysis.
    arguments:
    - h5dat : h5py data file object
    - ms : 0 or 1
    - path : the path of the adwin data group inside the container (from root)
    """
    ...
    rodata = np.load(basepath+'_'+RO_SUFFIX+'.npz')                 #NOTE here different data import function
    spdata = np.load(basepath+'_'+SP_SUFFIX+'.npz')                 #NOTE here different data import function
    crdata = np.load(basepath+'_'+CR_SUFFIX+'.npz')                 #NOTE here different data import function
