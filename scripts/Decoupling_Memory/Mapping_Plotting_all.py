...
def plot_carbon_using_all_values(ax,carbon_nr):
    ...
    color = carbon_params[C_str]['cl']
    label = carbon_params[C_str]['label']
    ax.scatter([x],[y],[z],c=color, marker=u'o', linewidths=0.01, label=label, s=np.pi * (6 * np.ones((3,)))**2)
...
posneg = ['neg','pos','pos','neg','neg']
carbons = [2,1,5,3,6]
# carbons = [2]
# carbons = []
for aa, carbon in enumerate(carbons):
    # for posneg1 in ['pos','neg']:
        posneg1 = posneg[aa]
        C_str = 'C' + str(carbon)
        plot_hf_circle(ax,carbon,posneg=posneg1)
...
plt.savefig(r'/Users/'+os.getlogin()+r'/Dropbox/Thesis/Images/hyperfinecircles.png',dpi=400)
plt.show()
