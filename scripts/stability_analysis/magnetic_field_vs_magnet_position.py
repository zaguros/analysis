...
def plot_meas_B_loop(step=0.3):
    ...
    # splt.ylim(440s,500)
    plt.ylim(np.max(relative_freq)-120,np.max(relative_freq)+60)
    # plt.xlim(0,220)
    ...
    ax.set_ylabel('freq (kHz) (offset 1.746 GHz)')
    # splt.ylim(440s,500)
    plt.ylim(np.min(relative_freq)-60,np.min(relative_freq)+120)
    # plt.xlim(0,220)
    plt.title('Magnetic field vs time')
    plt.savefig(folder+'/freq_vs_time.png',format='png')
...
# ## plot temperature corrected magnetic field vs measurement number

    # print 'iteration_index =' +str(iteration_index)

    fig = plt.figure(3,figsize=(18,5))
    ax = fig.add_subplot(111)
    ax.errorbar(iteration_index,relative_freq_corrected,u_f0m_filtered*1e6*10)
    ...
    f_bl = [] 
    j = 6
    f0m_reversed = list(reversed(f0m))
    Back_lash = []
    ...
    for i in range (num_turning_points):
        # f_bl.append(f0m_reversed[j-1])
        # f_bl.append(f0m_reversed[j])
        # f_bl.append(f0m_reversed[j+1])
        delta_f = (f0m_reversed[j]-f0m_reversed[j+1])/(f0m_reversed[j]-f0m_reversed[j-1])
        Back_lash.append((1-delta_f)*step)
        for k in range (11):
            f_average[k] = f_average[k] + ((f0m_reversed[k+j-5])/num_turning_points)
            f_average_b[k] = f_average_b[k] + ((f0m_reversed[k+j+12-5])/num_turning_points)
        j = j+24 
