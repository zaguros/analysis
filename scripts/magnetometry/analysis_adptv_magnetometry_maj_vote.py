...
class PhaseDistributionArray ():
	...
	def plot_scaling (self):
		...
		SQL_scaling=self.var[0]/(self.T/self.T[0])
		H_scaling=self.var[0]/((self.T/self.T[0])**2)
		plt.loglog (self.T, self.T*self.var, 'ob')
		plt.loglog(self.T,self.T*H_scaling,'g')
		plt.loglog(self.T,self.T*SQL_scaling,'r')
		plt.xlabel ('total estimation time')
		plt.ylabel ('variance * T ((B/g)**2*Hz)')
		plt.show()
		
		plt.loglog (self.T, self.T*self.holevo_var, 'ob')
		plt.loglog(self.T,self.T*H_scaling,'g')
		plt.loglog(self.T,self.T*SQL_scaling,'r')
		plt.xlabel ('total estimation time')
...
class RamseySequence():
	...
	def plot_avg_phase_distribution (self, max_rep = None):
		...
		for j in np.arange(max_rep):
			if ( np.mod(10000*j/max_rep, 1000) == 0):
				print 100*j/max_rep, '%'
			beta, prob = self.analysis (corrected=False, repetition = j)
			if (j==0):
				prob_avg = prob
			else:
	...
	def plot_single_phase_distributions (self, max_rep = None):
		if (max_rep==None):
			max_rep = self.reps
		for j in np.arange(max_rep):
			if ( np.mod(10000*j/max_rep, 1000) == 0):
	...
	def phase_distribution_scaling (self, max_rep = None, do_plot=False):
		...
		for n in np.arange(self.N-1)+2:
			print '############', n
			for j in np.arange(max_rep):
				beta, prob = self.analysis (corrected = False, N_max = n, repetition = j)
				if (j==0):
					p_avg = prob
				else:
					p_avg = p_avg+prob
...
class RamseySequence_Simulation (RamseySequence):
	...
	def setup_simulation(self, magnetic_field_hz = 0.):
	...
	def sim_cappellaro (self, debug = False):
		...
		for r in np.arange(self.reps):
			...
			for i in np.arange(self.N):
				...
				if debug:
					print '------ msmnt_step: ', self.curr_msmnt
...
def table_maker():
	...
	for n in np.arange(4)+9:
		print '@@@@@@@@@@@@@@@@@@@@@', n, '@@@@@@@@@@@@@@@@@@@@@@@'
		table = generate_table (N=n)
		print
		print
		print table
		np.savez('/home/cristian/Desktop/cappellaro_ideal_N='+str(n)+'.npz', table=table)
		
	
#table = generate_table (N=5)
#print table

#def simulations ():
#s = RamseySequence_Simulation (N_msmnts = 8, reps = 10, tau0=1e-9)
#s.setup_simulation(magnetic_field_hz =0e6)
#s.sim_non_adaptive (F=8, MK=12, K=5)

	#s.plot_avg_phase_distribution()
#	p = s.phase_distribution_scaling()
#	p.plot_distribution()
#	p.plot_scaling()

f = toolbox.latest_data(contains='121638')
...
s.print_phases()
p = s.phase_distribution_scaling (max_rep=100, do_plot=True)
p.plot_scaling()
