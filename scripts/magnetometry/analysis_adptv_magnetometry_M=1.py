...
class PhaseDistributionArray ():
	...
	def plot_scaling (self):
		...
		plt.loglog (self.T, self.var, 'ob')
		plt.loglog(self.T,H_scaling,'g')
		plt.loglog(self.T,SQL_scaling,'r')
		plt.xlabel ('total estimation time')
		plt.ylabel ('variance')
		plt.show()
		
		plt.loglog (self.T, self.holevo_var, 'ob')
		plt.xlabel ('total estimation time')
		plt.ylabel ('holevo_variance')
		plt.show()
