...
for g in a.g.items():    
    ...
    fig.savefig(os.path.join(folder, 'sp_relaxation_Th_pre={}_Th_pro={}'.format(int(pre),int(pro))))

plt.close('all')
a.finish()

sortidxs = argsort(ths)
ths = np.array(ths)[sortidxs][:]
...
res = fit.fit1d(ths, ks, common.fit_saturation, 120, 50,
    ret=True, do_print=True)


fig = plt.figure(figsize=(4,4))
ax = fig.add_subplot(111)
ax.errorbar(ths, ks, yerr=u_ks, fmt='o', capsize=0, elinewidth=2)

ax.set_xlabel('Threshold')
ax.set_ylabel('relaxation rate (kHz)')
ax.set_title(a.timestamp + '\nThreshold')

fig.savefig(os.path.join(folder, 'threshold'))
