...
def Rabi_basic(contains = '',timestamp=None, measurement_name = ['adwindata'],ssro_folder = None,
           Rabifreq = [0], scale = [0],
            fixed = [], ylim = [-0.5, 1.05],ssro_tstamp ='',base_folder = None,
            plot_fit = False, do_print = False, show_guess = True,correct_ionization = True, save =False):
    ''' Function to fit mbi-type data with exponential and sinusoidal functions or combinations thereof.
    ...
    for k in range(0,len(measurement_name)):
        ...
        p0, fitfunc, fitfunc_str = common.fit_Rabi(Rabifreq[0], scale[0])
        if show_guess:
            ax.plot(np.linspace(x[0],x[-1],201), fitfunc(np.linspace(0,x[-1],201)), lw=2)
        print 'starting fit.fit1d'
        fit_result = fit.fit1d(x,y, None, p0=p0, fitfunc=fitfunc, do_print=do_print, ret=True,fixed=fixed)
...
def NMR_basic(contains = '',timestamp=None, measurement_name = ['adwindata'],ssro_folder = None,
           Rabifreq = [0], scale = [0], x0 = [0], pulse_t = [0],offset = [0], inverse = False,
            fixed = [], ylim = [-0.5, 1.05],ssro_tstamp ='',base_folder = None,
            plot_fit = False, do_print = False, show_guess = True,correct_ionization = True, save =False):
    ''' Function to fit mbi-type data with exponential and sinusoidal functions or combinations thereof.
    timestamp       : format yyyymmdd_hhmmss or hhmmss or None. None takes the last data.
    ...
    for k in range(0,len(measurement_name)):
        ...
        y = a.p0.reshape(-1)[:]
        if inverse:
            p0, fitfunc, fitfunc_str = common.fit_inverse_NMR_spectrum(Rabifreq[0], scale[0], x0[0], pulse_t[0],offset[0])
        else:
            p0, fitfunc, fitfunc_str = common.fit_NMR_spectrum(Rabifreq[0], scale[0], x0[0], pulse_t[0],offset[0])
        if show_guess:
            # ax.plot(np.linspace(x[0],x[-1],201), fitfunc(np.linspace(0,x[-1],201)), lw=2)
...
def RF_phase_sweep(contains = '',timestamp=None, measurement_name = ['adwindata'],ssro_folder = None,
           phi0 = [0], scale = [0],
            fixed = [], ylim = [-0.5, 1.05],ssro_tstamp ='',base_folder = None,
            plot_fit = False, do_print = False, show_guess = True,correct_ionization = True, save =False):
    ''' Function to fit mbi-type data with exponential and sinusoidal functions or combinations thereof.
    ...
    for k in range(0,len(measurement_name)):
        ...
        p0, fitfunc, fitfunc_str = common.fit_phase_sweep(phi0[0], scale[0])
        if show_guess:
            # ax.plot(np.linspace(x[0],x[-1],201), fitfunc(np.linspace(0,x[-1],201)), lw=2)
            ax.plot(np.linspace(x[0],x[-1],201), fitfunc(np.linspace(x[0],x[-1],201)), lw=2)
