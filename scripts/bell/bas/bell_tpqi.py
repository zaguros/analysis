import time
folder = r'D:\measuring\data\2015-03-18-lhfbt2\BS'#r'D:\measuring\data\2015-04-13-lhfbt3\BS'#'D:\measuring\data\2014-12-Entanglement_XX_lock\BS'#r'D:\measuring\data\2014-11-Entanglement_XX_data\BS'# r'D:\measuring\data\21411_ZZ\BS'#


filepaths = tb.get_all_msmt_filepaths(folder, pattern='') 
co = np.ones([1,4])
...
if True:
    for i,f in enumerate(filepaths):
        ...
        pqf.close()
    fn = '{}_coincidences_with_laser_pulse'.format(time.strftime('%Y%m%d_%H%M'))
...
if True:
    pts=1
    Xs=np.linspace(2,8,pts)
    Vs=zeros(pts)
    Tots=zeros(pts)
    for i,x in enumerate(Xs):
        st_start_ch0 = 5444000
        st_len   = 30*1000. #50 ns
    ...
    if pts>1:
        ...
        ax=plt.subplot(111)
        ax.plot(Xs,Vs)
        ax.set_xlabel('windows start after 5444 [ns]')
