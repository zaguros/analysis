...
for kk in range(len(timestamp)):
    ...
    if kk == 0:
        ...
        cum_u_p0 = np.coatenate((cum_u_p0, a.u_p0))
...
if show_guess:
    ax.plot(np.linspace(0,x[-1]+2e4,201), fitfunc(np.linspace(0,x[-1]+2e4,201)), '-', lw=2)

fit_result = fit.fit1d(x,y, None, p0=p0, fitfunc=fitfunc, do_print=True, ret=True,fixed=[0,2])

## plot data and fit as function of total time
if plot_fit == True:
    plot.plot_fit1d(fit_result, np.linspace(0,x[-1]+2e4,201), ax=ax, plot_data=False)
