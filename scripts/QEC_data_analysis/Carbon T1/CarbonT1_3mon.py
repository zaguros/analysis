...
color_list = ['y','y']
x_max_list = [400e-3,10000e-3]
figure_name_list = ['C'+str(carbonnr)+'_T1_ms0','C'+str(carbonnr)+'_T1_ms1']
spacing = [100e-3]+[2000e-3]
...
for ii in range(2):
    ...
    # print y_err
    # print y[-1]-y_err[-1]
    # # fit_result['params_dict']['T'] =-10/np.log((y[-1]-y_err[-1]-0.5)/0.42645)
    # # fit_result['params_dict']['n'] =1.

    # print fit_result
    T_1 = -10/np.log((y[-1]-y_err[-1]-0.5)/0.42645)
    print 'T_1', T_1
    ...
    # Uncomment for ms=0
    xvals=np.linspace(0.,x_max_list[ii],201)
    plt.plot(xvals,0.5+0.42645*np.exp(-xvals / T_1),linewidth=2, linestyle = '--', color='0.25')
