...
def load_data (folder, timestamp, scan_type, return_folder = False):
    ...
    Thereafter it picks the 'hdf5' file from this folder (i.e. the file that contains the data) and selects the x and y data 
...
def fit_piezo_plots(date, timestamp, folder, V_min, V_max, set_range_V = False, save_data = False, threshold = 0.1,show_plots=True):
    ...
    for i in range(len(timestamp)):
        ...
        if show_plots:
            ...
            plot.plot_fit1d(fit_result, np.linspace(V[0],V[-1],10*len(V)), ax=ax,color='r',show_guess=True, plot_data=True, label = 'fit', add_txt=False)
            #ax.plot(f,function, 'g', label='testplot')
            ax.legend()
            # if indicate_FWHM:
            #   plt.plot(V[Half_Max_r], max(function)/2+offset, 'go')
            #   plt.plot(V[Half_Max_l], max(function)/2+offset, 'go')
            ax.set_xlabel('Voltage [V]',fontsize=14)
            ax.set_ylabel('Transmission (arb. units)',fontsize=14)
            ax.tick_params(axis = 'both', which = 'major',labelsize = 14)
            ax.set_xlim(V_min,V_max)
            ax.set_title(data_folder + ' \n Cavity linewidth in length is %s $\pm$ %s nm' %(round(Linewidth*182,2), round(u_Linewidth*182,3)))


            if save_data == True:
                fig.savefig(data_folder +'/'+ time + "piezofit " + ".png")
            plt.show()
            plt.close()
    ...
    variance_LW = sum([(x-average_LW)**2 for x in LW])/len(LW)
    st_dev_LW = np.sqrt(variance_LW)
    print  10*'*' + 'Average linewidth is ', round(average_LW*182,2), '+-', round(st_dev_LW*182,2), ' nm '+ 10*'*'


    determine_drift(times,peak_positions, data_folder)

def fit_birefringence(date, timestamp, folder, V_min, V_max, set_range_V = False, save_data = False, threshold = 0.1,show_plots=True):
    'This function plots all fine laser scans within the given timestamp range'
    print '### YOU\'RE ANALYSING PIEZO SCANS ### \n'
    LW1 = []
    LW2 = []
    ...
    for i in range(len(timestamp)):
        ...
        p0, fitfunc, fitfunc_str = common.fit_2lorentz(offset, amplitude, x0, gamma, amplitude2, x02, gamma2)
        fit_result = fit.fit1d(V,y, None, p0=p0, fitfunc=fitfunc, do_print=False, ret=True,fixed=fixed)
        ...
        As1.append(np.abs(A1))
        As2.append(np.abs(A2))
        ...
        if show_plots:
            ...
            ax.tick_params(axis = 'both', which = 'major',labelsize = 14)
            ax.set_xlim(V_min,V_max)
            ax.set_title(data_folder + ' \n Cavity linewidth in length is %s $\pm$ %s nm' %(round(gamma*182,2), round(u_gamma1*182,3)))
    ...
    print  10*'*' + 'Average linewidth is ', round(average_LW1*182,2), '+-', round(st_dev_LW1*182,2), ' nm '+ 10*'*'
    print  10*'*' + 'Average linewidth is ', round(average_LW2*182,2), '+-', round(st_dev_LW2*182,2), ' nm '+ 10*'*'

    determine_drift(times,peak_positions1, data_folder)
    determine_drift(times,peak_positions2, data_folder)

    plot_peak_intensities(As1,As2,polarizations)
...
def determine_drift(times, peak_positions, data_folder):
    ...
    fit_result = fit.fit1d(times_dec,peak_positions, None, p0=p0, fitfunc=fitfunc, do_print=True, ret=True,fixed=fixed)
    ...
    ax.set_title(data_folder + ' \n Drift is {} mV/s'.format(round(b*1.e3,2)))

    fig.savefig(data_folder +'/'+ time + "drift "  + ".png")
...
def plot_peak_intensities(As1,As2,polarizations):
    p_old = 0
    p_indices = []
    avg_As1 = []
    avg_As2 = []
    ...
    polarizations_values = [polarizations[i] for i in p_indices]
    ...
    print polarizations_values
    print len(polarizations_values)
    print avg_As1
    print len(avg_As1)
    print avg_As2
    ...
    ax.plot(polarizations_values,avg_As2, 'o', label = 'peak 2' )
    ax.plot(polarizations_values,avg_As1, 'o',label = 'peak 1' )
