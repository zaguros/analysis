...
''' Enter your data '''
date = '20150727'
measurement_type = 'LT_tubehigh_sync600ms'
scan_type = ''
# 'piezo_scans'
...
if fitting == True:
	...
	for i in range(len(timestamp)):
		...
		sigma.append(2*st_dev)
		X1 = mean - (st_dev)
		X2 = mean + (st_dev)


		threshold = np.where(y > max(y)/2)
		V1 = V[threshold[0][0]]
		V2 = V[threshold[0][-1]]
		LW_volt = V2-V1
		LW.append(LW_volt)
		...
		plt.title(time + '\n' + measurement_type + '\n Linewidth is %s V' %(round(LW_volt,5)))
...
# 	# print Linewidth/1e9
	...
	'''
	# For threshold (V2-V1)
	average_LW_volt = sum(LW)/float(len(LW))
	variance_LW_volt = sum([(x-average_LW_volt)**2 for x in LW])/len(LW)
	stdev_LW_volt = np.sqrt(variance_LW_volt)



	# For 2*sigma
	average_LW_sigma = sum(sigma)/float(len(sigma))
	variance_LW_sigma = sum([(x-average_LW_sigma)**2 for x in sigma])/len(sigma)
	stdev_LW_sigma = np.sqrt(variance_LW_sigma)
	...
	print 'AVERAGE LINEWIDTH IS ', round(average_LW_volt,5), 'V WITH A ST DEV OF ', round(stdev_LW_volt,5), 'V --- OR ', round(average_LW_sigma, 5), 'V WITH A ST DEV OF ', round(stdev_LW_sigma, 5), 'V'
